...
class DummyListener
{
    ...
    public function doAction(\ManaPHP\Events\Event $event, $component, $data)
    {
        $this->_testCase->assertEquals('doAction',$event->getType());
        $this->_testCase->assertInstanceOf('ManaPHP\Events\Event', $event);
...
class EventsTest extends TestCase{
    public function test_attachEvent(){
        ...
        $component=new DummyComponent();
        $that=$this;
        $component->attachEvent('dummy:doAction',function(\ManaPHP\Events\Event $event,$source,$data) use($that){
            $that->assertEquals('doAction',$event->getType());
            $that->assertInstanceOf('ManaPHP\Events\Event',$event);
